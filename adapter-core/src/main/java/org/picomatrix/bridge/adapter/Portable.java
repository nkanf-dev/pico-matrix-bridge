...
public final class Portable {
    ...
    public static String hex(byte[] bytes) { char[] chars=new char[bytes.length*2];String digits="0123456789abcdef";for(int i=0;i<bytes.length;i++){chars[i*2]=digits.charAt((bytes[i]&255)>>>4);chars[i*2+1]=digits.charAt(bytes[i]&15);}return new String(chars); }
    public static byte[] unhex(String value) { require(value.length()%2==0,"Invalid hex length");byte[] result=new byte[value.length()/2];for(int i=0;i<result.length;i++){int a=Character.digit(value.charAt(i*2),16),b=Character.digit(value.charAt(i*2+1),16);require(a>=0&&b>=0,"Invalid hex");result[i]=(byte)(a*16+b);}return result; }
    public static String sha(byte[] bytes) throws Exception { return hex(MessageDigest.getInstance("SHA-256").digest(bytes)); }
    public static String sha(Path path) throws Exception { MessageDigest d=MessageDigest.getInstance("SHA-256");try(InputStream in=Files.newInputStream(path)){byte[] b=new byte[65536];int n;while((n=in.read(b))!=-1)d.update(b,0,n);}return hex(d.digest()); }
    ...
    public static Set<String> keys(JSONObject object) { Set<String> result=new HashSet<>();Iterator<String> it=object.keys();while(it.hasNext())result.add(it.next());return result; }
