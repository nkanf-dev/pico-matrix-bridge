...
应用会自动检测。不依赖 Matrix 的应用照常安装；识别到 Matrix 接入时，可以选择通用 profile 适配，也可以安装原版。需要特殊处理的应用使用更具体的 profile，可独立于 Lab 更新。遇到应用兼容问题，可以在 issue 中提供应用名称、版本、头显型号和具体表现。新增 profile 请参阅[贡献指南](CONTRIBUTING.md)。
