...
Each profile lives in `profiles/<key>/`, with `manifest.json`, its recipe, `code/`, `apk/`, and `scripts/build.py`. A profile owns its application-specific matching and adaptation code; the bridge provides shared detection, APK preparation, installation, and recovery.
...
2. Add a manifest with a unique key, package matcher, and priority. Use the `generic` profile for shared PICO account integration; add a specific profile when an app needs its own changes. The build script must accept `--variant release --output <new-directory>` and write `matrix-profile-<key>.apk` there.
...
4. For local integration, sign profiles with a key you control and pass its public certificate to `profiles/<key>/scripts/build.py` with `--development-profile-certificate`. Pass the same certificate to `build_lab.py`. Never commit a private key or a built APK.
